...
export default function BunkPlanner() {
  ...
  // 🧠 SYNCED LOGIC: Calculates "Bunks Left" or "Classes to Attend"
  const getDayStatus = (day) => {
    ...
    const details = subjectsInDay.map(subj => {
      ...
      if (currentPct >= 75) {
        // Formula: How many can we skip? (Attended / 0.75) - Total
        const canSkip = Math.floor((attended / 0.75) - total);
        bunkAnalysis.count = canSkip;
        bunkAnalysis.message = canSkip > 0 ? `${canSkip} Bunks Available` : "Critical Limit";
        bunkAnalysis.type = "BUNK";
        bunkAnalysis.isDangerous = canSkip <= 0;
      } else {
        // Formula: How many to attend? (0.75 * Total - Attended) / 0.25
        const mustAttend = Math.ceil((0.75 * total - attended) / 0.25);
        bunkAnalysis.count = mustAttend;
        bunkAnalysis.message = `Attend ${mustAttend} More`;
        bunkAnalysis.type = "ATTEND";
        bunkAnalysis.isDangerous = true;
      }

      if (bunkAnalysis.isDangerous) criticalCount++;
  ...
  return (
    <div className={`min-h-screen p-4 md:p-10 font-sans transition-colors duration-500 ${dark ? 'bg-[#0b0f1a] text-white' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <header className="glass p-8 rounded-[40px] border border-white/10 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 mt-2">75% Threshold Sync Active</p>
            </div>
          </div>
          
          <button 
            onClick={() => setIsEditing(!isEditing)} 
            className={`btn-glass px-10 py-4 flex items-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all ${isEditing ? 'bg-emerald-500 text-white' : ''}`}
          >
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="glass p-8 rounded-[40px] border border-white/20 shadow-2xl mb-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white/5 rounded-3xl mb-8">
                  <select value={selectedDay} onChange={e => setSelectedDay(e.target.value)} className="input-glass p-3 font-bold uppercase text-[10px] tracking-widest bg-transparent outline-none">
                    {DAYS.map(d => <option key={d} value={d} className="text-black">{d}</option>)}
                  </select>
                  <select value={subjectToAdd} onChange={e => setSubjectToAdd(e.target.value)} className="input-glass p-3 font-bold uppercase text-[10px] tracking-widest bg-transparent outline-none">
                  ...
                  <button onClick={handleAddSubjectToDay} className="bg-primary-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-lg">Add Class</button>
                </div>
          ...
          <div className="space-y-6">
            {processedDays.length === 0 ? (
              <div className="text-center py-20 opacity-20">
                ...
                <p className="font-black uppercase tracking-widest text-xs">Define your timetable to see bunk analysis</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-30">{item.details.length} Classes Scheduled</p>
                      </div>
                    </div>
                    ...
                    <div className={`px-6 py-3 rounded-2xl border font-black text-[10px] uppercase tracking-[0.2em] flex items-center gap-3 ${
                      ...
                      'border-red-500/30 bg-red-500/10 text-red-400'
                       ...
                       {item.status === 'SAFE' ? 'Safe to Bunk' : 
                        item.status === 'RISKY' ? 'High Alert' : 'No Bunk Zone'}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8 pt-8 border-t border-white/5">
                    {item.details.map((d, j) => (
                      <div key={j} className={`p-5 rounded-[30px] bg-black/20 border border-white/5 ${d.isDangerous ? 'border-red-500/30' : 'border-emerald-500/20'}`}>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-30 mb-3 truncate">{d.subject}</p>
                        
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                             <p className="text-[8px] font-black opacity-30 uppercase">Current</p>
                             <p className={`text-xs font-black ${d.isDangerous ? 'text-red-400' : 'text-emerald-400'}`}>{d.currentPct}%</p>
                          </div>
                          
                          <div className="bg-white/5 p-3 rounded-2xl flex items-center justify-between">
                            <div>
                               <p className="text-[7px] font-black opacity-40 uppercase mb-0.5">Advice</p>
                               <p className={`text-[10px] font-black uppercase tracking-tighter ${d.isDangerous ? 'text-red-400' : 'text-emerald-400'}`}>
                                 {d.message}
                               </p>
                            </div>
                            <div className={`p-2 rounded-lg ${d.isDangerous ? 'bg-red-500/20 text-red-500' : 'bg-emerald-500/20 text-emerald-500'}`}>
                               {d.type === 'BUNK' ? <HiOutlineTrendingUp /> : <HiOutlineTrendingDown />}
                            </div>
                          </div>
                        </div>
                      </div>
