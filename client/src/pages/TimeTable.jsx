...
import api from "../utils/api"; // Using your new API utility
...
export default function TimeTable() {
  ...
  // Load schedule and start live timer
  useEffect(() => {
    fetchSchedule();
  ...
  const addSlot = async () => {
    ...
    try {
      ...
      setShowAdd(false);
      setEntry(initialState); // Reset form logic from new code
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 relative transition-colors duration-500 ${dark ? "text-white" : "text-slate-900"}`}>
      {/* Decorative Background Blob from Old UI */}
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-[10px] font-black tracking-[0.4em] opacity-40 uppercase mt-2">
              Database Connected • Live Status Active
            </p>
          </div>
          <button
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <select
                  ...
                  className={`input-glass ${dark ? "text-white" : "text-black"}`}
                >
                  {days.map((d) => (
              ...
              <button
                onClick={addSlot}
                className="w-full py-4 bg-cyan-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-cyan-600 transition-colors"
              >
                Publish to Database
              </button>
            </motion.div>
            ...
            <div key={day} className="space-y-4">
              ...
              <div className="flex flex-nowrap overflow-x-auto gap-4 pb-4 scrollbar-hide">
                {schedule.filter((s) => s.day === day).length > 0 ? (
                    ...
                    .map((slot) => {
                      ...
                      return (
                        <motion.div
                          key={slot._id}
                          whileHover={{ y: -5 }}
                          className={`min-w-[280px] glass p-6 rounded-[35px] border transition-all relative group ${
                            isLive
                              ? "border-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.3)] bg-cyan-500/5"
                              : "border-white/10"
                          }`}
                        >
                          {isLive && (
                            <div className="absolute top-4 right-4 text-cyan-400 animate-pulse flex items-center gap-1 font-black text-[8px] uppercase tracking-widest">
                              <HiOutlineStatusOnline /> Live Now
                            </div>
                          )}
                          ...
                          <h4 className={`text-lg font-black uppercase tracking-tight mb-4 ${isLive ? "text-cyan-400" : ""}`}>
                            {slot.subject || "Lunch Break"}
                          </h4>
                          <div className="space-y-2 opacity-60 text-[10px] font-bold">
                            <div className="flex items-center gap-2">
                              <HiOutlineClock className="text-cyan-500" /> {slot.startTime} - {slot.endTime}
                            </div>
                            {slot.type !== "Lunch" && (
                              <>
                                <div className="flex items-center gap-2">
                                  <HiOutlineLocationMarker className="text-cyan-500" /> Room: {slot.room}
                                </div>
                                <div className="flex items-center gap-2">
                                  <HiOutlineUser className="text-cyan-500" /> {slot.faculty}
                                </div>
                              </>
                            )}
                          </div>
                  ...
                  <div className="text-[10px] font-black uppercase tracking-widest opacity-20 py-6">
                    Free Schedule
                  </div>
                )}
              </div>
